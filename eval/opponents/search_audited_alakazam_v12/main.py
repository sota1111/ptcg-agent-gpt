...
class AttackPlan:
    def __init__(
        self, attacker=-1, target=-1, attack_index=-1, remain_hp=-1, needs_energy=False
    ):
...
class AdvancedPolicy:
    def __init__(self, obs: Observation):
        ...
        self.can_switch, self.can_gust, self.can_attack, self.can_use_mega_brave = (
            False,
            False,
            False,
            False,
    ...
    def _can_evolve_board_index(self, board_index: int) -> bool:
        for option in self.select.option:
            ...
            target_index = option.inPlayIndex + (
                1 if option.inPlayArea == AreaType.BENCH else 0
            )
        ...
        for attacker_index, my_pokemon in enumerate(self._my_board()):
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    score += (
                        base_score
                        + (220 if attacker_index == 0 else 0)
    ...
    def _score_card_choice(self, option) -> float:
        ...
        if self.context == SelectContext.SETUP_ACTIVE_POKEMON:
            return (
                2
                if card.id == C.SOLROCK and self.state.firstPlayer == self.my_index
                else 4
        ...
        if self.context == SelectContext.TO_HAND:
            ...
            if card.id == C.MAKUHITA:
                score += (
                    (80 if self.field_counts[card.id] < 2 else -20)
                    if self._opponent_is_crustle_wall()
                    else (-10 if self.field_counts[card.id] >= 1 else 10)
                )
            elif card.id == C.HARIYAMA:
                score += (
                    (120 if self.field_counts[C.MAKUHITA] >= 1 else -5)
                    if self._opponent_is_crustle_wall()
                ...
                score += (
                    -150
                    if (
                        self.field_counts[C.RIOLU]
                        + self.field_counts[C.MEGA_LUCARIO_EX]
                        >= 2
                    )
                    else -3
                    if (
    ...
    def _score_play(self, option) -> float:
        ...
        if data.cardType == CardType.POKEMON:
            ...
            if (
                card.id == C.RIOLU
                and self.field_counts[C.RIOLU] + self.field_counts[C.MEGA_LUCARIO_EX]
                >= 2
            ):
                return -1
            return 20000
        ...
        if card.id == C.CARMINE:
            if self._opponent_is_crustle_wall() and any(
                c.id in {C.HARIYAMA, C.MAKUHITA} for c in self.me.hand
            ):
                return -1
...
def evaluate_state(obs):
    ...
    is_crustle = any(
        p is not None and p.id in {344, 345}
        for p in [op.active[0] if op.active else None] + list(op.bench)
    )
    is_snorlax = any(
        p is not None and p.id == 143
        for p in [op.active[0] if op.active else None] + list(op.bench)
    )
    ...
    for p in [op.active[0] if op.active else None] + list(op.bench):
        ...
        if p.id == C.MEGA_LUCARIO_EX:
            op_dmg = (
                270 if assumed_energies >= 2 else 130 if assumed_energies >= 1 else 0
            )
        elif p.id == C.HARIYAMA:
            ...
            op_dmg = 130 if assumed_energies >= 2 else 0  # Dragapult ex
        else:
...
def SEARCH_ALGO(obs_dict, obs):
    ...
    try:
        ...
        while time.time() - t0 < SEARCH_TIME_BUDGET:
            ...
            valid_scores = [
                total_val[a] / visits[a] for a in candidates if visits[a] > 0
            ]
            if not valid_scores:
                break
