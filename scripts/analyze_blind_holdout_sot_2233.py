"""Deterministically summarize the SOT-2233 blind holdout reports."""
...
def main() -> int:
    ...
    reports = {
        row["label"]: json.loads(
            (root / "artifacts" / "sot-2233" / "holdout" / f"{row['label']}.json").read_text()
        )
    ...
    result = {
        ...
        "report_sha256": {
            label: sha256(root / "artifacts" / "sot-2233" / "holdout" / f"{label}.json")
