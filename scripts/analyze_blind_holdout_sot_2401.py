"""Validate and summarize the preregistered SOT-2401 blind terminal audit."""
...
def main() -> int:
    ...
    report_dir = root / "artifacts/sot-2401/holdout"
    reports = {
    ...
    for opponent in manifest["opponents"]:
        ...
        deck_matches = sha(Path(opponent["repo"]) / "deck.csv") == opponent["deck_sha256"]
        if current_commit != opponent["commit"] or not deck_matches:
            raise ValueError(f"{opponent['label']}: opponent provenance drift")
    provenance = manifest["provenance"]
    paths = {
        "main_sha256": root / "main.py",
        "deck_sha256": root / "deck.csv",
        "candidate_artifact_sha256": root / "agents/public_belief.py",
        "decision_sha256": root / "artifacts/sot-2400/screen-decision.json",
        "source_manifest_sha256": root / "eval/manifests/sot-2399-lb-reanchoring.json",
    }
    ...
    subprocess.run(
        ["git", "merge-base", "--is-ancestor", provenance["sot_2400_terminal_commit"], "HEAD"],
        cwd=root,
        check=True,
    )
